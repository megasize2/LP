...
const shadowStrongStyle = {
  textShadow: "0 0 15px rgba(59, 130, 246, 0.6), 0 2px 5px rgba(0, 0, 0, 0.9)",
}
...
export default function SkillsSection() {
  return (
    ...
    >
      ...
      <div className="absolute top-1/4 right-0 w-96 h-96 bg-blue-600/10 rounded-full mix-blend-multiply filter blur-3xl opacity-15"></div>
      <div className="absolute bottom-1/4 left-0 w-96 h-96 bg-indigo-600/10 rounded-full mix-blend-multiply filter blur-3xl opacity-15"></div>
      ...
      <div className="container mx-auto px-4 md:px-12 relative z-10">
        <ScrollReveal direction="scale">
          <h2 className="text-2xl md:text-4xl mobile-heading text-center mb-8 md:mb-16 relative">
            <span className="block text-sm text-blue-300 mb-1 md:mb-2 uppercase tracking-wider font-medium bg-black/60 py-1 px-3 rounded-lg inline-block shadow-lg">
              Qualifications
            </span>
            <span className="digital-text text-3xl md:text-5xl relative section-title block mt-2">
              <span className="bg-black/70 py-2 px-4 rounded-lg shadow-lg inline-block">Skills & Certifications</span>
              {/* 装飾ライン */}
              <span className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 w-20 h-1 bg-blue-500/70"></span>
            </span>
          </h2>
        </ScrollReveal>
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10">
          ...
          <ScrollReveal direction="flip" delay={100}>
            <div className="futuristic-card p-5 md:p-8 rounded-xl hover-lift important-section">
              ...
              <h3
                className="text-lg md:text-xl font-semibold mb-3 md:mb-4 highlight-text important-text text-blue-300"
                style={shadowStrongStyle}
              >
                資格
              </h3>
              <ul className="space-y-3 md:space-y-4">
                {["AIパスポート（取得学習中）", "普通運転免許", "情報処理士", "1級船舶免許"].map((item, index) => (
                  <li key={index} className="flex items-center group data-display p-2 rounded-md">
                    ...
                    <span
                      className="group-hover:text-white transition-colors mobile-readable text-outline text-white font-medium"
